...
String클래스는 `불변(immutable)`이라는 특성을 가지고 있습니다.
...
"Hello World!"예제를 볼때 "Hello"에 + " World!"를 결합해서 문자열 인스턴스에 할당된 값을 "Hello World!"로 변경했다고 생각하실 수 있습니다.  
하지만 인스턴스 내의 문자열이 바뀐 것이 아니라 새로운 문자열이 담긴 String인스턴스가 생성된 것입니다. str은 새로운 주소값을 가리키게 되고 기존의 "Hello"가 할당된 메모리공간은 Garbage에 이전됐다가 GC에 의해 소멸하게 됩니다.

> 💡 문자열을 위의 예제처럼 결합하면 매 연산 시 마다 새로운 String인스턴스가 생성됩니다. 이는 메모리공간을 지속적으로 차지하게 되므로 결합을 최대한 줄여야합니다.  
> 이런 작업이 많이 필요할 때 사용할 수 있는 StringBuffer가 있는데 이는 다음에 알아보도록 하겠습니다.

# 2. String 타입의 변수 선언과 할당 방법
