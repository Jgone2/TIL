...
위의 식처럼 `1 + 2 + 3`을 작성하면 계산만 되고 계산된 결과는 사라진다. 우리는 결과값을 재사용하거나 추후 출력하기 위해서는 어딘가에 저장을 해둬야한다. 바로 이전 게시글에서 공부했던 `변수`에 담아야한다. 이때 대입 연산자 `=`를 사용하는데 변수에 대입할 때는 대입할 값은 무조건 변수의 오른쪽에 위치해야 한다.
...
증감 연산자는 피연산자를 1씩 즈악 혹은 감소 시킬 때 사용하는 연산자다. 증감연산자의 피연산자는 한 개이며 증감연산자가 피연산자의 앞에 위치하느냐 뒤에 위치하느냐에 따라 결과값이 달라진다.
...
위의 예제처럼 증감 연산자의 위치에 따라 결과값이 완전히 달라지기 때문에 복잡한 연산이라면 오차는 더욱 커질 것이다. 이 점에 유의하여 사용해야 한다.

## 2. 산술 연산자

산술 연산자는 우리가 평소에 자주 사용하는 사칙연산을 다루는 연산자들이 포함되어 있다.
...
평소에 자주 사용하는 사칙연산이지만 `/`를 눈여겨 봐야한다. 우리가 평소에 알던 나눗셈의 결과값으로는 `4.33333333333`이 나와야하는데 그냥 숫자`4`가 나왔다.
이것은 나누기의 두 연산자의 자료형이 정수형인 int이기 때문이다. 정수형이기 때문에 소수점 아래의 자리는 저장하지 못하므로 정수만 출력되는 것이다. 나눗셈 결과로 소수점을 버릴 때 반올림은 하지 않는다.

그렇다면 정확한 값을 얻기 위해서는 어떻게 해야할까?
바로 이전에 공부한 `형변환`을 사용해야 한다.
...
비트 연산자는 10진수의 연산보다는 2진수의 연산에 주로 사용하거나 1의 보수를 만들 때 사용한다.
...
다음 예제는 1byte크기를 기준으로 8bit로 연산하는 예제다.
...
비교 연산자는 두 피연산자를 비교하는 데 사용되는 연산자다. 주로 조건문과 반복문의 조건식에 사용되며, 연산결과는 논리형인 `boolean`형(true, false)으로 출력된다.
...
논리 연산자는 주어진 논리식을 판단하여, 참(true)과 거짓(false)을 결정하여 `boolean`형의 결과값을 가지는 연산자.
...
대입 연산자는 변수에 값을 대입할 때 사용하는 연산자다. 자바에서는 대입 연산자와 다른 연산자를 결합한 복합 대입 연산자가 제공한다.
...
기타 연산자로는 대표적으로 삼항 연산자와 instanceof 연산자가 있다.
...
인스턴스에 대해서는 추후에 객체에 대해 설명을 할 때 자세히 설명하도록 하겠다.
